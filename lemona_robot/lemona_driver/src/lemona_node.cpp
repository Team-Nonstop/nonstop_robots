...
#define BATTERY_LOW_VOLATGE		12
#define BATTERT_EMPTY			11 
...
void robotDataCallback(std::string * data){ 
    ...
    if (confirm_communication){
      ...
      ROS_INFO("Lemona robot is Streaming Data.");
...
int main(int argc, char** argv){ //typical usage: "./lemona_node /dev/ttyACMx"
  
	ros::init(argc, argv, "lemona_node");
	ros::NodeHandle n;
	ros::NodeHandle pn("~");
	std::string port;
	...
	ROS_INFO("Starting Lemona robot...");
	...
	int VDriver, Lemona_Firmware, Robot_Voltage;
	sscanf(reply.c_str(), "@5,%d,%d,%d,%de", &Lemona_Firmware, &Robot_Voltage, &VDriver, &ID_Robot);   //encoder msg parsing

	ROS_INFO("Lemona robot ID = %d", ID_Robot);
	if (ID_Robot < 4 || ID_Robot > 5){
		ROS_WARN("Attention! Unexpected Lemona robot ID!");
	}
	ROS_INFO("Lemona Firmware = %.2f", Lemona_Firmware*0.01);
	...
	ROS_INFO("Robot_Voltage = %.2f V", Robot_Voltage*0.01 );
	if (Robot_Voltage < 100){
		ROS_ERROR("Robot is off. Check power source!");
		return(0);
	}
	if (Robot_Voltage < 850 && Robot_Voltage > 100){
		ROS_WARN("Warning! Low Robot_Voltage!");
	}
