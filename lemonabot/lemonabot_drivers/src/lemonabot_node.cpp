...
cereal::cereal_port serial_port;
...
int main(int argc, char** argv){ //typical usage: "./Lemonabot_node /dev/ttyACMx"
  
	ros::init(argc, argv, "Lemonabot_node");
	ros::NodeHandle n;
	ros::NodeHandle pn("~");
	std::string port;
	...
	sscanf(reply.c_str(), "@5,%de", &ID_Robot);   //encoder msg parsing

	ROS_INFO("Lemonabot ID = %d", ID_Robot);
	if (ID_Robot < 4 || ID_Robot > 5){
		ROS_WARN("Attention! Unexpected Lemonabot ID!");
	}
