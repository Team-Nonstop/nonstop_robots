...
void SniperNode::tablePosesCB(const semantic_region_handler::TablePoseList::ConstPtr& msg)
{
...
void SniperNode::deliverOrderCB()
{
  if (order_.status != cafe_msgs::Status::IDLE)
  {
    ROS_WARN("sniperbot not in idle status; cannot attend request (current status is %s)", toCStr(order_.status));
//    return;
  ...
  order_process_thread_ = boost::thread(&SniperNode::processOrder, this, order_);
}

void SniperNode::preemptOrderCB()
{
...
//< DEBUG
void SniperNode::fakeOrderForEasyDebugging(int order_id, int table_id)
{
...
//  if (order_id == 4)       order_.status = cafe_msgs::Status::IDLE;
  ...
  if (order_id == 1)       boost::thread wakeUpThread(&SniperNode::wakeUp, this);
  if (order_id == 2)       boost::thread wakeUpThread(&SniperNode::leaveNest, this);
