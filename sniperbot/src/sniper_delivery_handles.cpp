...
bool SniperNode::processOrder(cafe_msgs::Order& order)
{
  ...
  {
    return setFailure("Sniper failed to go to pickup place");
  }
  ...
  {
    return setFailure("Sniper didn't receive the button from kitchen");
  }
  ...
  {
    return setFailure("Sniper failed to go to table");
  }
  ...
  if (waitForButton() == false)
  {
    return setFailure("Sniper didn't receive the button from customer");
  }
