...
{
  ros::init(argc, argv, "sniperbot");

  sniperbot::SniperNode node(ros::this_node::getName());
  if (node.init() == false)
  {
    ROS_ERROR("%s sniper down", ros::this_node::getName().c_str());
    return -1;
  }
  ROS_INFO("Sniper in ready");
